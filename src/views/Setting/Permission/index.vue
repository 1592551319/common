<template>
  <div class="app-home">
    <ColorPicker v-model="color1" style="width: 200px" />
    <Button type="default" @click="handleRed">按钮</Button>
    <Button type="primary" @click="handleOrange">按钮</Button>
  </div>
</template>
...
export default {
  ...
  mounted() {
    // document.documentElement.style.cssText = "--primary-color: #e53935";
    // ['ed4014', '237,64,20', '#ef532c', '#f16643', '#f2795b', '#f48c72', '#f6a08a', '#f8b3a1', '#fac6b9', '#fbd9d0', '#fdece8', '#d53a12']
    // ['fb8c00', '251,140,0', '#fb981a', '#fca333', '#fcaf4d', '#fdba66', '#fdc680', '#fdd199', '#feddb3', '#fee8cc', '#fff4e6', '#e27e0']
  },
  methods: {
    handleRed() {
      this.handleSetColor('#ed4014', '#9c27b0')
    },
    handleOrange() {
      this.handleSetColor('#ed4014', '#fb8c00')
    },
    ...
    async setThemeColor(newval, oldval) {
      ...
      // 获取DOM元素上的style
      const domAll = [].slice.call(document.getElementsByTagName('*')).filter((dom) => {
        const stylCon = dom.getAttribute('style')
        return (
          stylCon &&
					(new RegExp(oldval, 'i').test(stylCon) || orignalRGBRegExp.test(stylCon))
        )
      })
      domAll.forEach((dom) => {
